...
def readArff(filehandle) :
    ...
    for index in range(len(attribLines)) :
        ...
        # chunks = line.split(' ',2)
        chunks = re.split("[ ]+",line,2)
        if chunks[2].startswith('{') : ### this is a nominal attribute
            # print chunks
            attributes[index] = {chunks[1] : [item.strip().strip('{}').strip("\'") for item in chunks[2].strip('\n{} ').split(',')]}
        else : ## this is string or numeric
            attributes[index]= {chunks[1] : chunks[2]}
    classification = attributes.pop(attributes.keys()[len(attributes)-1])
    ### data will be lines that don't begin with a '@'
    dataLines = [line.strip() for line in lines if not line.startswith('@')]
    for line in dataLines :
        data.append([i.strip().strip("\'") for i in line.split(',')])
    return (attributes, data, classification)
...
if __name__ == '__main__' :
    if len(sys.argv) < 2 :
        ...
        sys.exit(-1)
    fname = sys.argv[-1]
    (attrs, data, classification) = readArff(open(fname))
